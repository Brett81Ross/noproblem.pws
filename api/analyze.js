...
export default async function handler(req, res) {
    ...
    try {
        ...
        let outputProposalString = "PROJECT DIAGNOSES SCAN REPORT:\n\n";
        let subtotal = 0;
        ...
        if (scanData.services && Array.isArray(scanData.services)) {
            scanData.services.forEach((item) => {
                ...
                outputProposalString += `- ${spec.label}: $${itemTotal.toFixed(2)} (${item.quantity} ${spec.unit})\n`;
            });
        }

        if (subtotal < rateCard.minimumJob && subtotal > 0) {
            subtotal = rateCard.minimumJob;
            outputProposalString += `\n* Minimum job order adjustment enforcement applied.`;
        }

        outputProposalString += `\nTOTAL PROPOSAL EXPORT: $${subtotal.toFixed(2)}`;
